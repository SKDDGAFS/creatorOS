...
from app.platforms.instagram import (
    OAuthStartResponse as InstagramOAuthStartResponse,
)
...
from app.platforms.tiktok import OAuthStartResponse as TikTokOAuthStartResponse
from app.platforms.youtube import OAuthStartResponse as YouTubeOAuthStartResponse
from app.schemas.channel import ChannelResponse
from app.schemas.platform_integration import (
    InstagramOAuthStartRequest,
    InstagramPublishingLimitResponse,
    PlatformAccountMetricSnapshotResponse,
    PlatformConnectionResponse,
    PlatformQuotaUsageResponse,
    TikTokOAuthStartRequest,
    VideoSyncResponse,
...
@router.post(
    ...
    response_model=InstagramOAuthStartResponse,
    status_code=status.HTTP_201_CREATED,
)
def start_instagram_oauth(
    ...
    return instagram_service.begin_authorization(
        db,
        secret_store=secret_store,
        settings=settings,
        workspace_id=context.workspace_id,
...
def complete_instagram_oauth(
    ...
    adapter_factory: Annotated[
        InstagramAdapterFactory,
        Depends(get_instagram_adapter_factory),
    ],
    state_value: Annotated[
        str,
        Query(alias="state", min_length=20, max_length=500),
    ...
    return instagram_service.complete_authorization(
        db,
        secret_store=secret_store,
        adapter_factory=adapter_factory,
        state=state_value,
...
def get_instagram_connection(
    ...
    return instagram_service.get_connection(
        db,
        workspace_id=context.workspace_id,
        connection_id=connection_id,
    )
...
@router.post(
    "/instagram/{connection_id}/sync/channel",
    response_model=ChannelResponse,
)
def sync_instagram_channel(
    connection_id: UUID,
    context: Annotated[WorkspaceContext, Depends(require_workspace_write)],
    db: Annotated[Session, Depends(get_db)],
    secret_store: Annotated[
    ...
    return instagram_service.sync_channel(
        db,
        workspace_id=context.workspace_id,
        connection_id=connection_id,
        secret_store=secret_store,
...
@router.post(
    "/instagram/{connection_id}/sync/videos",
    response_model=VideoSyncResponse,
)
def sync_instagram_videos(
    connection_id: UUID,
    context: Annotated[WorkspaceContext, Depends(require_workspace_write)],
    db: Annotated[Session, Depends(get_db)],
    secret_store: Annotated[
    ...
    result = instagram_service.sync_videos(
        db,
        workspace_id=context.workspace_id,
        connection_id=connection_id,
        secret_store=secret_store,
...
@router.post(
    "/instagram/{connection_id}/sync/account-insights",
    response_model=list[PlatformAccountMetricSnapshotResponse],
)
def sync_instagram_account_insights(
    connection_id: UUID,
    context: Annotated[WorkspaceContext, Depends(require_workspace_write)],
    db: Annotated[Session, Depends(get_db)],
    secret_store: Annotated[
    ...
    return list(
        instagram_service.sync_account_metrics(
            db,
            workspace_id=context.workspace_id,
            connection_id=connection_id,
            secret_store=secret_store,
...
@router.post(
    "/instagram/{connection_id}/sync/videos/{video_id}/metrics",
    response_model=list[VideoMetricResponse],
)
...
@router.get(
    "/instagram/{connection_id}/publishing-limit",
    response_model=InstagramPublishingLimitResponse,
)
def get_instagram_publishing_limit(
    ...
    result = instagram_service.get_publishing_limit(
        db,
        workspace_id=context.workspace_id,
        connection_id=connection_id,
        secret_store=secret_store,
    ...
    return InstagramPublishingLimitResponse(
        quota_usage=result.quota_usage,
        quota_total=result.quota_total,
        quota_duration_seconds=result.quota_duration_seconds,
    )


@router.get(
    "/instagram/{connection_id}/quota",
    response_model=list[PlatformQuotaUsageResponse],
)
def list_instagram_quota(
    connection_id: UUID,
    context: Annotated[WorkspaceContext, Depends(get_workspace_context)],
    db: Annotated[Session, Depends(get_db)],
    start_date: date | None = None,
    ...
    instagram_service.get_connection(
        db,
        workspace_id=context.workspace_id,
        connection_id=connection_id,
    )
...
def disconnect_instagram(
    ...
    return instagram_service.disconnect(
        db,
        workspace_id=context.workspace_id,
        connection_id=connection_id,
        secret_store=secret_store,
...
@router.post(
    "/tiktok/oauth/start",
    response_model=TikTokOAuthStartResponse,
    status_code=status.HTTP_201_CREATED,
)
def start_tiktok_oauth(
    payload: TikTokOAuthStartRequest,
    context: Annotated[WorkspaceContext, Depends(require_workspace_admin)],
    db: Annotated[Session, Depends(get_db)],
    secret_store: Annotated[
        InMemoryPlatformSecretStore,
    ...
    return tiktok_service.begin_authorization(
        db,
        secret_store=secret_store,
        settings=settings,
        workspace_id=context.workspace_id,
...
def complete_tiktok_oauth(
    ...
    adapter_factory: Annotated[
        TikTokAdapterFactory,
        Depends(get_tiktok_adapter_factory),
    ],
    state_value: Annotated[
        str,
        Query(alias="state", min_length=20, max_length=500),
    ...
    return tiktok_service.complete_authorization(
        db,
        secret_store=secret_store,
        adapter_factory=adapter_factory,
        state=state_value,
...
@router.get(
    "/tiktok/{connection_id}",
    response_model=PlatformConnectionResponse,
)
def get_tiktok_connection(
    connection_id: UUID,
    context: Annotated[WorkspaceContext, Depends(get_workspace_context)],
    db: Annotated[Session, Depends(get_db)],
) -> PlatformConnection:
    return tiktok_service.get_connection(
        db,
        workspace_id=context.workspace_id,
        connection_id=connection_id,
    )
...
@router.post(
    "/tiktok/{connection_id}/sync/channel",
    response_model=ChannelResponse,
)
def sync_tiktok_channel(
    connection_id: UUID,
    context: Annotated[WorkspaceContext, Depends(require_workspace_write)],
    db: Annotated[Session, Depends(get_db)],
    secret_store: Annotated[
    ...
    return tiktok_service.sync_channel(
        db,
        workspace_id=context.workspace_id,
        connection_id=connection_id,
        secret_store=secret_store,
...
@router.post(
    "/tiktok/{connection_id}/sync/videos",
    response_model=VideoSyncResponse,
)
def sync_tiktok_videos(
    connection_id: UUID,
    context: Annotated[WorkspaceContext, Depends(require_workspace_write)],
    db: Annotated[Session, Depends(get_db)],
    secret_store: Annotated[
    ...
    result = tiktok_service.sync_videos(
        db,
        workspace_id=context.workspace_id,
        connection_id=connection_id,
        secret_store=secret_store,
...
@router.post(
    "/tiktok/{connection_id}/sync/account-insights",
    response_model=list[PlatformAccountMetricSnapshotResponse],
)
def sync_tiktok_account_insights(
    connection_id: UUID,
    context: Annotated[WorkspaceContext, Depends(require_workspace_write)],
    db: Annotated[Session, Depends(get_db)],
    secret_store: Annotated[
    ...
    return list(
        tiktok_service.sync_account_metrics(
            db,
            workspace_id=context.workspace_id,
            connection_id=connection_id,
            secret_store=secret_store,
...
@router.post(
    "/tiktok/{connection_id}/sync/videos/{video_id}/metrics",
    response_model=list[VideoMetricResponse],
)
def sync_tiktok_metrics(
    connection_id: UUID,
    video_id: UUID,
    context: Annotated[WorkspaceContext, Depends(require_workspace_write)],
    db: Annotated[Session, Depends(get_db)],
...
@router.get(
    "/tiktok/{connection_id}/quota",
    response_model=list[PlatformQuotaUsageResponse],
)
def list_tiktok_quota(
    connection_id: UUID,
    context: Annotated[WorkspaceContext, Depends(get_workspace_context)],
    db: Annotated[Session, Depends(get_db)],
    start_date: date | None = None,
...
def disconnect_tiktok(
    ...
    return tiktok_service.disconnect(
        db,
        workspace_id=context.workspace_id,
        connection_id=connection_id,
        secret_store=secret_store,
