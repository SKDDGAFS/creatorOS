...
from app.platforms.runtime import InstagramAdapterFactory
from app.schemas.video_metric import (
    InstagramMetricExtensionCreate,
    VideoMetricCreate,
)
...
def _get_instagram_connection(
    ...
    *,
    ...
    if connection.platform != Platform.INSTAGRAM.value:
        raise InvalidRequestError("Connection is not an Instagram connection")
    if require_connected and connection.status != ConnectionStatus.CONNECTED.value:
        raise ConflictError("Instagram connection must be reconnected")
    return connection
    ...
    *,
    ...
    if isinstance(error, PlatformCredentialExpiredError):
        if (
            ...
            _commit(db, "Unable to update Instagram connection status")
    ...
    raise PersistenceError("Instagram request failed") from error
...
def _invoke[Result](
    ...
    *,
    ...
    adapter_factory: InstagramAdapterFactory,
    operation: Callable[[InstagramAdapter, CredentialMaterial], Result],
) -> Result:
    quota = QuotaAccumulator()
    requests = RequestLogAccumulator()
    ...
    try:
        ...
        _commit(db, "Unable to update Instagram connection status")
        raise ConflictError(
            "Instagram credentials are unavailable; reconnect the account"
        ) from exc
    try:
    ...
    *,
    ...
    try:
        if provider_error is not None:
            raise AuthorizationError("Instagram authorization was not completed")
        if authorization_code is None or not authorization_code.strip():
            raise InvalidRequestError("Instagram authorization code is missing")
        adapter = adapter_factory(quota.record, requests.record)
        try:
        ...
        if not set(claimed.requested_scopes).issubset(credentials.scopes):
            ...
            raise AuthorizationError(
                "Meta did not grant all requested Instagram permissions"
            )
        ...
        existing = db.scalar(
            select(PlatformConnection).where(
                ...
                PlatformConnection.platform == Platform.INSTAGRAM.value,
        ...
        try:
            if existing is None:
                ...
                _commit(db, "Unable to reconnect Instagram account")
        except Exception:
...
def _upsert_channel(
    ...
    *,
    ...
    _commit(db, "Unable to synchronize Instagram account")
    db.refresh(channel)
    return channel
...
def sync_channel(
    ...
    *,
    ...
    connection = _get_instagram_connection(
        db,
        workspace_id=workspace_id,
        connection_id=connection_id,
    )
...
def sync_videos(
    ...
    *,
    ...
    _commit(db, "Unable to synchronize Instagram media")
...
def _metric_payload(snapshot: RemoteMetricSnapshot) -> VideoMetricCreate:
    extension_data = snapshot.metadata.get("instagram_extension")
    extension = (
        InstagramMetricExtensionCreate.model_validate(extension_data)
        if isinstance(extension_data, dict)
        else InstagramMetricExtensionCreate()
    )
    ...
    try:
        return VideoMetricCreate(
            ...
            instagram_extension=extension,
        )
    except ValidationError as exc:
        raise InvalidRequestError(
            "Instagram returned analytics outside supported ranges"
        ) from exc
...
def sync_account_metrics(
    ...
    *,
    ...
    _commit(db, "Unable to synchronize Instagram account insights")
...
def get_publishing_limit(
    ...
    *,
    ...
    secret_store: CredentialStore,
    adapter_factory: InstagramAdapterFactory,
) -> PublishingLimitResult:
    connection = _get_instagram_connection(
        db,
        workspace_id=workspace_id,
        connection_id=connection_id,
    )
    usage, total, duration = _invoke(
        db,
        connection=connection,
        secret_store=secret_store,
        adapter_factory=adapter_factory,
        operation=lambda adapter, credentials: adapter.get_publishing_limit(
            connection.external_account_id,
            credentials,
        ),
    )
    return PublishingLimitResult(
        quota_usage=usage,
        quota_total=total,
        quota_duration_seconds=duration,
    )
...
def disconnect(
    ...
    *,
    ...
    try:
        ...
        _commit(db, "Unable to update Instagram connection status")
        raise ConflictError(
            "Instagram credentials are unavailable; reconnect before disconnecting"
        ) from exc
...
def get_connection(
    ...
    *,
    ...
    return _get_instagram_connection(
        db,
        workspace_id=workspace_id,
        connection_id=connection_id,
        require_connected=False,
